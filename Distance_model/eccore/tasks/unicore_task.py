...
import torch
from eccore import metrics, utils
from eccore.data import UnicoreDataset, data_utils, iterators
...
class UnicoreTask(object):
    ...
    def dataset(self, split):
        ...
        Returns:
            a :class:`~eccore.data.UnicoreDataset` corresponding to *split*
        """
        from eccore.data import UnicoreDataset
    ...
    def get_batch_iterator(
        ...
        Args:
            dataset (~eccore.data.UnicoreDataset): dataset to batch
            ...
            disable_iterator_cache (bool, optional): don't cache the
                ...
                (default: False).
        Returns:
            ~eccore.iterators.EpochBatchIterator: a batched iterator over the
                given dataset split
        """
    ...
    def build_model(self, args: Namespace):
        """
        Build the :class:`~eccore.models.BaseUnicoreModel` instance for this
        task.

        Returns:
            a :class:`~eccore.models.BaseUnicoreModel` instance
        """
        from eccore import models
        return models.build_model(args, self)

    def build_loss(self, args: Namespace):
        """
        Build the :class:`~eccore.losses.UnicoreLoss` instance for
        this task.
        ...
        Returns:
            a :class:`~eccore.losses.UnicoreLoss` instance
        """
        from eccore import losses
    ...
    def train_step(
        ...
        Args:
            sample (dict): the mini-batch. The format is defined by the
                :class:`~eccore.data.UnicoreDataset`.
            model (~eccore.models.BaseUnicoreModel): the model
            loss (~eccore.losses.UnicoreLoss): the loss
            optimizer (~eccore.optim.UnicoreOptimizer): the optimizer
        ...
        with torch.autograd.profiler.record_function("forward"):
            ...
            #orig_grad_norm = clip_grad_norm_(model.parameters(), 10000)
    ...
    def valid_step(self, sample, model, loss, test=False):
        ...
        with torch.no_grad():
            if test:
                loss, sample_size, logging_output = loss.forward_sample(model, sample)
            else:
                loss, sample_size, logging_output = loss.forward(model, sample)
        return loss, sample_size, logging_output
