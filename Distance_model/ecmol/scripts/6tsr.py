...
# Utils - adapted from public Unimol: ecmol.utils.conf_gen_cal_metrics
...
# Utils - from public Unimol: ecmol.utils.docking_utils
...
# Utils - from public Unimol: ecmol.utils.conformer_model
...
def single_refine(
    ...
    in_lig_list = Chem.rdmolfiles.SDMolSupplier(input_ligand_path, sanitize=False) 

    #print('input_ligand_path:', input_ligand_path)
    #print('output_ligand_path:', output_ligand_path)
    
    filename = os.path.basename(output_ligand_path)
    pathname = os.path.dirname(output_ligand_path)
    output_ligand_path = os.path.join(pathname, 'clash_fix_optimize_' + filename) #
    #sdf
    w = Chem.SDWriter(output_ligand_path)

    for in_lig in in_lig_list:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Number of trials for confgen + 6t superimposition. Keeps the lowest RMSD") #superimposition
    parser.add_argument(
