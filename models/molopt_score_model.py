...
def get_refine_net(refine_net_type, config, equiformer_args = None, escn_args = None):
    if refine_net_type == 'uni_o2':
        refine_net = UniTransformerO2TwoUpdateGeneral(
            ...
            k=config.knn,
            edge_feat_dim=config.edge_feat_dim, #？，4，4，8，8*11
...
class ScorePosNet3D(nn.Module):
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
        ...
        if sample == False:
            ...
            if len(init_ligand_v.shape) == 1:
                init_ligand_v = F.one_hot(init_ligand_v, self.num_classes).float() #one-hot。index_to_log_onehot
            ...
            if self.time_emb_dim > 0:  #，，，，
                if self.time_emb_mode == 'simple':
                    input_ligand_feat = torch.cat([
                        init_ligand_v,
                        (time_step / self.num_timesteps)[batch_ligand].unsqueeze(-1) #time_step / self.num_timestep：[0,1)，
                        #
                    ], -1)
                elif self.time_emb_mode == 'sin': #+1mlp
            ...
            if self.config.node_indicator: #0
            ...
            #print('')
            #，.，，，，
            #ligand_v = F.one_hot(init_ligand_v, self.num_classes).float().detach().requires_grad_(True) #one-hot,
            ligand_v = index_to_log_onehot(init_ligand_v, self.num_classes).float().detach().requires_grad_(True) #one-hot，one_hot

            init_h_protein = self.protein_atom_emb(protein_v) #protein_vone-hot
        ...
        if self.config.diffusion_mode == 'CM':
            #，sigma.sigma. mlp，sigma，
            #sigma_emb = self.sigma_emb_layer(sigmas.view(-1,1)).unsqueeze(1)

            sigma_emb = self.sigma_emb_layer(sigmas.view(-1,1)) # shape = num_graph * hidden_dim, h.shape  = num_graph, num_node, hidden_dim

            #sigma，sigma，batch_ligand
        ...
        #GNN，
        pos_all, org_pos_all = pos_all / rate, org_pos_all / rate #
        ...
        outputs['x'], org_pos_all = outputs['x'] * rate, org_pos_all * rate #
        ...
        #final_ligand_v = self.v_inference(final_ligand_h) #MLP
        #final_ligand_v = torch.tensor(0.0).cuda()
        #final_ligand_v = 0
        final_ligand_v = torch.zeros([final_ligand_pos.size(0)]).cuda()
        
        if 'pred_exp_from_all' in self.config and self.config.pred_exp_from_all:  #false，
            ...
            final_ligand_h: torch.Size([99, 49, 16]) #，99*16
            atom_affinity: torch.Size([99, 49])
            '''

            atom_affinity = self.expert_pred(final_ligand_h).squeeze(-1) #mlp，
    ...
    def get_diffusion_loss(
        ...
        # Vt = a * V0 + (1-a) / K
        log_ligand_v0 = index_to_log_onehot(ligand_v, self.num_classes) #one-hot
        #ligand_v_perturbed, log_ligand_vt = self.q_v_sample(log_ligand_v0, time_step, batch_ligand) #
        ligand_v_perturbed = log_ligand_v0 #，，
        log_ligand_vt = 0

        # 3. forward-pass NN, feed perturbed pos and v, output noise, posv
        preds = self.forward(
        ...
        pred_pos_noise = pred_ligand_pos - ligand_pos_perturbed  #pos？

        # atom position， ？？pos，？，model_mean_typeC0，pos_model_mean
        #C0：， noise：
        ...
        #
        rmsd = self.py_rmsd(target, pred) #rmsd，。
        
        #，
        next_cp_ligand_fill_coords    = ligand_fill_coords.clone()
        current_cp_ligand_fill_coords = ligand_fill_coords.clone()

        next_cp_ligand_fill_coords[ligand_fill_masks==True]      = pred[ligand_fill_atom_order] #zmats，
    ...
    def sample_diffusion(
        ...
        init_protein_pos, init_ligand_pos, offset = center_pos(protein_pos, init_ligand_pos, batch_protein, batch_ligand, mode=center_pos_mode) #
        org_protein_pos, org_init_ligand_pos, org_offset = center_pos(protein_pos, org_ligand_pos, batch_protein, batch_ligand, mode=center_pos_mode) #
        ...
        #print('813?:', self.num_classes) #13
        

        org_ligand_pos  = copy.deepcopy(org_init_ligand_pos) #KNN，，KNN
        org_protein_pos = copy.deepcopy(org_protein_pos) #KNN，，KNN
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            #origin_ligand_pos  = copy.deepcopy(ligand_pos) #KNN，KNN
            #origin_protein_pos = copy.deepcopy(protein_pos) #，
            
            if guide_mode == 'joint' or guide_mode == 'pdbbind_random': 
                #,，？
                #self.pv_joint_guide，。 preds。
            ...
            if self.model_mean_type == 'noise':
                ...
                #v0_from_e = preds['pred_ligand_v'] #preds['pred_ligand_v']，0，
            elif self.model_mean_type == 'C0':
            ...
            if guide_mode == 'joint' or guide_mode == 'pdbbind_random':
                ...
                #pos_model_mean = pos_model_mean + pos_grad_weight*(0.5 * pos_log_variance).exp()*pos_grad #，  pos_model_mean
                #log_ligand_v = log_ligand_v + type_grad_weight*type_grad
                
                ligand_pos_next = pos_model_mean + nonzero_mask * (0.5 * pos_log_variance).exp() * torch.randn_like(
                    ligand_pos)  #ddpm? ，X = μ +　Y*σ ，YN(0, 1)，（）
