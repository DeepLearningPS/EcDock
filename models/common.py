...
class MLP(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, hidden_dim, num_layer=2, norm=True, act_fn='relu', act_last=False):
        ##print('in_dim, out_dim, hidden_dim:', in_dim, out_dim, hidden_dim) #20 1 128
        ...
        layers = []
        #print('in_dim, out_dim, hidden_dim, num_layer:', in_dim, out_dim, hidden_dim, num_layer)
        for layer_idx in range(num_layer):
            if layer_idx == 0:
                layers.append(nn.Linear(in_dim, hidden_dim)) #20,128
            elif layer_idx == num_layer - 1:
                layers.append(nn.Linear(hidden_dim, out_dim)) #128,1
            else:
    ...
    def forward(self, x):
        ...
        ##print('in_dim, out_dim, hidden_dim:', self.in_dim, self.out_dim, self.hidden_dim)

        '''
        x: torch.Size([112483, 20])
        x: torch.Size([112483, 424])
        '''
        #print('x.shape1:', x.shape)
        rs = self.net(x) #RuntimeError: mat1 and mat2 shapes cannot be multiplied (103536x424 and 340x128)
        #print('x.shape2:', rs.shape)
...
def compose_context(h_protein, h_ligand, pos_protein, pos_ligand, batch_protein, batch_ligand, protein_element, ligand_element,
    ...
    atom_isring_ctx  = torch.cat([protein_atom_isring, ligand_atom_isring], dim=0)[sort_idx]
    atom_isO_ctx     = torch.cat([protein_atom_isO, ligand_atom_isO], dim=0)[sort_idx]
    atom_isN_ctx     = torch.cat([protein_atom_isN, ligand_atom_isN], dim=0)[sort_idx]


    cross_isring_ctx  = torch.cat([cross_pro_isring_flag, cross_lig_isring_flag], dim=0)[sort_idx]
    cross_isO_ctx     = torch.cat([cross_pro_isO_flag, cross_lig_isO_flag], dim=0)[sort_idx]
    cross_isN_ctx    = torch.cat([cross_pro_isN_flag,  cross_lig_isN_flag], dim=0)[sort_idx]
    cross_pos_ctx     = torch.cat([cross_protein, cross_ligand], dim=0)[sort_idx]  # (N_protein+N_ligand, 3)

    protein_coords_predict_ctx = torch.cat([pos_protein, protein_coords_predict], dim=0)[sort_idx]


    return h_ctx, pos_ctx, batch_ctx, element_ctx, atom_isring_ctx, atom_isO_ctx, atom_isN_ctx, cross_isring_ctx, cross_isO_ctx, cross_isN_ctx, cross_pos_ctx, mask_ligand, protein_coords_predict_ctx
