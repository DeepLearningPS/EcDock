...
class eSCN(BaseModel):
    ...
    Args:
        use_pbc (bool):         Use periodic boundary conditions # #，，KNN
    ...
    def __init__(
        ...
        if self.distance_function == "gaussian":
            self.distance_expansion = GaussianSmearing(
                0.0,
                cutoff,
                #self.num_gaussians, #8.0/0.02 = 400 #，
                int(self.num_gaussians / 8),  #50, #400/8 = 50, 8
    ...
    def forward(self,         
        ...
        #edge_rot_mat = self._init_edge_rot_mat(None, None, edge_distance_vec) #pos, edge_index, ，，None
        edge_rot_mat = self._init_edge_rot_mat(pos, edge_index, edge_distance_vec) #pos, edge_index, ，，None
        ...
        #print('x.embedding1:', x.embedding.shape)
        x.embedding = x.embedding + h.view(x.embedding.size())

        #
        #print('x.embedding2:', x.embedding.shape)
        #print('edge_distance1:', edge_distance.shape)
        ...
        #print('edge_distance3:', edge_distance.shape)

        '''
        x.embedding1: torch.Size([518, 49, 16])
        x.embedding2: torch.Size([518, 49, 16])
        edge_distance1: torch.Size([16434])
        edge_distance2: torch.Size([16434, 50])
        edge_distance3: torch.Size([16434, 400])
        '''
        ###############################################################
        ...
        #print('x.embedding3:', x.embedding.shape) #torch.Size([518, 49, 16])

        outputs = {"emb": x.embedding.view(x.embedding.size(0), -1)} #
        ...
        #print('x_pt1:', x_pt.shape) #x_pt: torch.Size([518, 16, 16]) 
        x_pt = x_pt.view(-1, self.sphere_channels_all)
        #print('x_pt2:', x_pt.shape) #x_pt: torch.Size([8288, 16])
        ...
        if self.show_timing_info is True:
            ...
            logging.info(
                f"{self.counter} Time: {time.time() - start_time}\tMemory: {len(pos)}\t{torch.cuda.max_memory_allocated() / 1000000}"
            )
    ...
    def _init_edge_rot_mat(self, pos, edge_index, edge_distance_vec):
        ...
        if torch.min(edge_vec_0_distance) < 0.0001:
            ...
            print('edge_index is None or []?:', edge_index) #edge_indexNone
            logging.error(
                f"Error edge_vec_0_distance: {minidx} {edge_index[0, minidx]} {edge_index[1, minidx]} {pos[edge_index[0, minidx]]} {pos[edge_index[1, minidx]]}"
            )
...
class EdgeBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = num_gaussians
        ...
        # Embedding function of the distance
        self.fc1_dist = nn.Linear(self.in_channels, self.edge_channels) #(16434x400 and 50x16)
    ...
    def forward(self, edge_distance, source_element, target_element):
        ...
        #edge_distance
        x_dist = edge_distance
        x_dist = self.fc1_dist(x_dist) #(16434x400 and 50x16)
