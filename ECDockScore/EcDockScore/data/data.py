...
class PDBbindDataset(Dataset):
    def __init__(self,
        ...
        if isinstance(ligs,np.ndarray) or isinstance(ligs,tuple) or isinstance(ligs,list):
            if isinstance(ligs[0],dgl.DGLGraph):  #DGL
                self.graphsl = ligs
            else:
    ...
    def __init__(self,
        ...
        if isinstance(ligs,np.ndarray) or isinstance(ligs,list):
            ...
            if ligs.endswith(".mol2"):
                ...
                #print('ligs:', ligs) #/mnt_191/fanzhiguang/47/VSDS/Docking_baseline/KarmaDock/vsds_gen/P00748-6X0S/decopy229/decopy229_pred_ff_corrected.sdf
                lig_blocks = self._sdf_split(ligs)
                #print('lig_blocks:', lig_blocks) #[]
                self.ligs = [Chem.MolFromMolBlock(lig_block) for lig_block in lig_blocks]
                #print('self.ligs:', self.ligs) #[]
                self.graphsl = self._mol_to_graph()
                #print('self.graphsl:', self.graphsl) #[]
            else:
                try:
                    self.graphsl,_ = load_graphs(ligs)
                except:
        ...
        #print('self.ligs, self.graphsl:', self.ligs, self.graphsl) #[] []
        if ids is None:
        ...
        #print('self.idsx, self.graphsl:', self.idsx, self.graphsl) #[] []
