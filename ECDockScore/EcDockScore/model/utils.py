...
def run_an_eval_epoch(model, data_loader, pred=False, atom_contribution=False, res_contribution=False, dist_threhold=None, aux_weight=0.001, device='cuda'):
    model.eval()
    total_loss = 0
    mdn_loss = 0
    atom_loss = 0
    ...
    with th.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            #print('bgp.device:', bgp.device)
            #print('bgl.device:', bgl.device)
            pi, sigma, mu, dist, atom_types, bond_types, batch = model(bgp, bgl) # pi，mu， sigma
            #dist？，？。。
...
def calculate_probablity(pi, sigma, mu, y):
    #pi, sigma, mu, dist：pi，mu， sigma, ydist
    normal = Normal(mu, sigma) #,
    logprob = normal.log_prob(y.expand_as(normal.loc)) #y，
    logprob += th.log(pi) # #，log(p1*p2)=log(p1)+log(p2)
    prob = logprob.exp().sum(1) #，，，。，，1。
