...
class EcDockScore(nn.Module):
    ...
    def forward(self, bgp, bgl):
        ...
        #C_batch = th.tensor(range(B)).unsqueeze(-1).unsqueeze(-1).cuda()
        C_batch = th.tensor(range(B)).unsqueeze(-1).unsqueeze(-1)
        #print('C_batch:', C_batch.device) #C_batch: cuda:0
        #print('C_mask:', C_mask.device)
