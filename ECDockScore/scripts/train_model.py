...
from EcDockScore.data.data import PDBbindDataset
from EcDockScore.model.model2 import EcDockScore, DGLGraphTransformer 
from EcDockScore.model.utils import collate, EarlyStopping, set_random_seed, run_a_train_epoch, run_an_eval_epoch, mdn_loss_fn
...
'''dgl：，rdkit mol。EcDockScore/feats/mol2graph_rdmda_res.py，，
rdkitdgl，pickle，pickle。'''

data = PDBbindDataset(ids="%s/%s_ids.npy"%(args["data_dir"], args["train_prefix"]), #id
                      ligs="%s/%s_l.bin"%(args["data_dir"], args["train_prefix"]), #pickle，dgl，dglpickle
                      prots="%s/%s_p.bin"%(args["data_dir"], args["train_prefix"])
                      )

train_inds, val_inds = data.train_and_test_split(valnum=args["valnum"], seed=args['seeds']) #id，
