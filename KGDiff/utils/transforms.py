...
from KGDiff.datasets.pl_data import ProteinLigandData
from KGDiff.utils import data as utils_data
...
class FeaturizeProteinAtom(object):
    ...
    def __init__(self):
        ...
        self.max_num_aa = 20 #，20，43，，，，。

    @property
    def feature_dim(self):
        return self.atomic_numbers.size(0) + self.max_num_aa + 1 #27

    def __call__(self, data: ProteinLigandData):
        element = data.protein_element.view(-1, 1) == self.atomic_numbers.view(1, -1)  # (N_atoms, N_elements)
        amino_acid = F.one_hot(data.protein_atom_to_aa_type, num_classes=self.max_num_aa) #one-hot
...
class FeaturizeLigandAtom(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        AROMATIC_FEAT_MAP_IDX = 2
        '''
        aromatic_list = [v[AROMATIC_FEAT_MAP_IDX] for v in data.ligand_atom_feature] #data.ligand_atom_feature.shape = N * 8, 
        #，，，1，0，data.ligand_atom_featureATOM_FAMILIES，one-hot
        ...
        x = torch.tensor(x)
        data.ligand_atom_feature_full = x #one-hot，one-hot，
        return data
