...
FOLLOW_BATCH = ('protein_element', 'ligand_element', 'ligand_bond_type', 'protein_link_t', 'protein_link_e') #batch，，
...
class ProteinLigandData(Data):
    ...
    def from_protein_ligand_dicts(protein_dict=None, ligand_dict=None, **kwargs):
        ...
        #？。 ligand_bond_index == ligand_edge_index. ，（），ligand_bond_index[0]
    ...
    def __inc__(self, key, value, *args, **kwargs): #kv？from_protein_ligand_dicts()？

        ##print('key:', key)
        ##print('value:', value)
        '''
         Python ，__inc__()  __index__() 。。，
        Python  __index__() （）。
        '''
...
def torchify_dict(data):
    ...
    for k, v in data.items():
        if isinstance(v, np.ndarray) and k not in ['cross_distance', 'cross_bond_index', 'cross_bond_type', 'cross_bond_index_reverse', 'cross_bond_type_reverse'][:1]:
