...
class ConsistencyTraining:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        c_lp_distance = self.calculate_distance_matrix(c_l_pos, c_p_pos) # n * m
        n_lp_distance = self.calculate_distance_matrix(n_l_pos, n_p_pos) # n * m
        ...
        if self.use_classifier_guide:  #The default is True
            ...
            loss = ic_loss * GP.loss_weight['ic'] + loss_pos * GP.loss_weight['xyz'] + cross_distance_loss + ligand_distance_loss
        else:
            #loss = ic_loss * GP.loss_weight['ic'] + loss_pos * GP.loss_weight['xyz']
            loss = ic_loss * GP.loss_weight['ic'] + loss_pos * GP.loss_weight['xyz'] + cross_distance_loss + ligand_distance_loss
...
class ConsistencySamplingAndEditing:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        time_list = np.array(list(reversed(list(range(len(sigmas)))))) + 1 #，1。0，
        ...
        ligand_pos = self.__mask_transform(preds['pred_ligand_pos'], y, mask, transform_fn, inverse_transform_fn)
