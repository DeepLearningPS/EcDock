...
class Equi_Consistency_Model_Parallel:
    ...
    def Load(self,modelname,loadtype='Perepoch'):
        ...
        with tempfile.TemporaryDirectory() as dirpath:
            ...
            if loadtype=='Minloss':
            ...
            elif loadtype=='Perepoch':  #，minlossepoch，，
                #print('load Perepoch model')
                ...
                raise Exception('not find model, please supply correct model path ')
    ...
    def IC_Loss(self,pred,target,zmats,gmasks):
        ...
        ##print('zmats.shape:', zmats.shape) #zmats.shape: torch.Size([1024, 9, 5]),zmats[*;*;4]? [*;*;5]? 
        pred_bonddis,pred_angle,pred_dihedral,j1,j2,j3=xyz2ic(pred,zmats) #
        ##print('pred_bonddis,pred_angle,pred_dihedral,j1,j2,j3.shape:', pred_bonddis.shape, pred_angle.shape, pred_dihedral.shape, j1.shape, j2.shape, j3.shape)

        #pred.shape: torch.Size([1024, 9, 3])
        #zmats.shape: torch.Size([1024, 9, 5])
        #pred_bonddis, pred_angle, pred_dihedral, j1, j2, j3.shape: 
        # torch.Size([1024, 9]) torch.Size([1024, 9]) torch.Size([1024, 9]) torch.Size([1024, 9, 3]) torch.Size([1024, 9, 3]) torch.Size([1024, 9, 3])
    ...
    def RMSD_Loss(self,pred,target,zmats,gmasks):
        ...
        pred_aligned = ic2xyz(bond,angle,dihedral,zmats) #？
        
        #target
        bond,angle,dihedral,*_ = xyz2ic(target,zmats)
        target_aligned = ic2xyz(bond,angle,dihedral,zmats) #？
    ...
    def Train_Step(self,Datas,step_id):
        ...
        zmats=Datas["Zmats"]
        gmasks=Datas["Masks"]
        bzids=torch.arange(zmats.shape[0]).view(-1,1).tile((1,zmats.shape[1])).unsqueeze(-1).long() #tensor.tile() == tensor.repeat()
        ##print('zmats2.shape:', zmats.shape) #torch.Size([1024, 9, 4])
        ##print('bzids.shape:', bzids.shape) #4N*4*1  torch.Size([1024, 9, 1])
        zmats=torch.concat((bzids,zmats),axis=-1) 
        ##print('zmats3.shape:', zmats.shape) #torch.Size([1024, 9, 5])
        #raise Exception('stop')
    ...
    def Fit(self,Train_MGFiles,Valid_MGFiles,Epochs=1000000):
        ...
        for epoch in range(Epochs):
            for file_id, Fname in enumerate(Train_MGFiles):
                ...
                ##print('len(Train_MGs):', len(Train_MGs)) #30000
                ##print('type(Train_MGs):', type(Train_MGs)) #list
                ##print('Train_MGs[0]:', Train_MGs[0])  #KGEcConf.graphs.mol.Molgraph object
                ##print('====================================')
                ...
                if self.local_rank is not None:
                    ...
                    Trainloader=DataLoader(Train_Dataset,batch_size=self.batchsize,shuffle=True,num_workers=GP.n_workers,) #batchsize = 1024 in QM9
                    #DataLoader

                ##print('len(Trainloader):', len(Trainloader)) #30
                ##print('Trainloader.type:', type(Trainloader)) #[dict,dict,...,dict]

                trainbar=tqdm(enumerate(Trainloader))
                self.train_epoch_loss=0
                ntrain_batchs=math.ceil(len(Train_MGs)/self.batchsize)
                ##print('ntrain_batchs:', ntrain_batchs) #30
                

                for bid,Datas in trainbar:
                    train_batch_loss=0
                    ##print('=======================')
                    ##print('type(Datas):', type(Datas)) #dict
                    ##print('Datas[Adjs].shape:', Datas["Adjs"].shape) #shape:torch.Size([1024, 9, 9])
                    ...
                    elif GP.random_step == 5: #，50,100
                        for step in range(GP.final_timesteps): #150，
                            step_loss,step_loss_dismat,step_loss_bonddis,step_loss_angle,step_loss_dihedral,step_lstr=self.Train_Step(Datas,step_id=step)
                            #pprint.p#print(Datas[''])
                            if self.local_rank==0 or self.local_rank is None:
                                lstr=f'Training -- Epochs: {epoch} file: {file_id} bid: {bid} step: {step} lr: {self.lr:.3E} '+step_lstr
