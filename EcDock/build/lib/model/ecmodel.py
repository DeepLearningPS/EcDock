...
class Equi_Consistency_Model:
    ...
    def Load(self,modelname,loadtype='Perepoch'):
        ...
        with tempfile.TemporaryDirectory() as dirpath:
            ...
            if loadtype=='Minloss':
                ...
                '''
                modelcpkt=torch.load(f"{dirpath}/model/online_model_minloss.cpk") #online_model_minloss
                ...
                modelcpkt=torch.load(f"{dirpath}/model/online_model_minloss.cpk") #online_model_minloss
                self.online_model.load_state_dict(parallel_cpkt_to_single(modelcpkt)) #GPU，，
    ...
    def sample_batch(self,Datas):
        ...
        with torch.no_grad():
            ...
            if GP.final_timesteps == 1:
                sigmas= reversed(sigmas)  #sigmas1，，sigmas，
                sigmas[-1] = 80 #80sigma，1，karras_schedule()sigma ，，，
            else:
    ...
    def Sample(self,MGs,conf_num_per_mol=10,savepath='./mol.sdf'):
        ...
        for MG in MGs: #
            Final_MGs+=[MG]*conf_num_per_mol #2
        ...
        if GP.multi_step == 1:
            ...
            for stp in list(total_samples.keys()):   
                total_samples[stp]=torch.concat(total_samples[stp],axis=0) #，
            ...
            for stp in list(total_samples.keys()):
                ...
                for i in range(len(Final_MGs)):    #
                    ...
                    sampled_coords=total_samples[stp][i].clone().detach().cpu().numpy()[:mg.natoms] #
                    mg.Update_Coords(sampled_coords) #
                    try:
                        mol=mg.Trans_to_Rdkitmol() #rdkit mol， #mol
                        #mol=mg.update_mol()
                        mols.append(mol)
                        #molsupp.write(mol)          #，mol
                    except:
                        pass
                #molsupp.close()
                mols_dict[stp] = mols
            ...
            for i in range(len(Final_MGs)):    #
                ...
                mg.Update_Coords(sampled_coords) #
                try:
                    mol=mg.Trans_to_Rdkitmol() #rdkit mol
                    #mol=mg.update_mol()
                    mols.append(mol)
                    #molsupp.write(mol)          #，mol
                except:
